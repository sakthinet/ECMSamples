...
{
    public partial class Form_bkup4 : Form
    {
        ...
        public Form_bkup4()
        {
            ...
            // Add event handlers explicitly in constructor to ensure they're connected
            cmbProductList.SelectedIndexChanged += cmbProductList_SelectedIndexChanged_1;
            cmdProductService.SelectedIndexChanged += cmdProductService_SelectedIndexChanged_1;
        }
        ...
        private void LoadECMProducts()
        {
            ...
            // Set the first item as selected to populate the second dropdown
        ...
        private void btnOpenURL_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selected accelerator is not a web application. Please use Open Accelerator button instead.");
            }
        }
        ...
        private void cmbProductList_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string product = cmbProductList.SelectedItem.ToString();
                    cmdProductService.Items.AddRange(ecmData[product].Keys.ToArray());

                    // Set the first service as selected to populate the accelerator dropdown
                    if (cmdProductService.Items.Count > 0)
                    {
                        cmdProductService.SelectedIndex = 0;
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error in cmbProductList_SelectedIndexChanged: {ex.Message}");
            }
        }

        private void cmdProductService_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            try
            {
                cmbAccelerator.Items.Clear();
                ...
                if (cmbProductList.SelectedItem != null && cmdProductService.SelectedItem != null)
                {
                    ...
                    if (ecmData.ContainsKey(product) && ecmData[product].ContainsKey(service))
                    {
                        cmbAccelerator.Items.AddRange(ecmData[product][service].Keys.ToArray());

                        // Set the first accelerator as selected
                        if (cmbAccelerator.Items.Count > 0)
                        {
            ...
            {
                MessageBox.Show($"Error in cmdProductService_SelectedIndexChanged: {ex.Message}");
            }

        }

        private void btnOpenApplication_Click_2(object sender, EventArgs e)
        {
            panel1.Visible = true;
